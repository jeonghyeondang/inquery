...
public class EasyLambdaQueryWrapper<T> extends AbstractLambdaWrapper<T, EasyLambdaQueryWrapper<T>>
    ...
    /**
     * SELECT some SQL settings with condition
     *
     * @param condition whether to apply
     * @param columns query fields
     */
    @Override
    public EasyLambdaQueryWrapper<T> select(boolean condition, List<SFunction<T, ?>> columns) {
        if (condition && com.baomidou.mybatisplus.core.toolkit.CollectionUtils.isNotEmpty(columns)) {
            this.sqlSelect.setStringValue(columnsToString(false, columns));
        }
        return typedThis;
    }
