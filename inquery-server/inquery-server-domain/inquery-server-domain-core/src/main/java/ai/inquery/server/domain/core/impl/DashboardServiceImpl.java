...
import ai.inquery.server.domain.core.converter.DashboardConverter;
...
import ai.inquery.server.domain.repository.mapper.DashboardMapper;
...
public class DashboardServiceImpl implements DashboardService {
    ...
    @Override
    public DataResult<String> enableShare(Long id) {
        Dashboard dashboard = queryExistent(id).getData();
        PermissionUtils.checkOperationPermission(dashboard.getUserId());

        String token = dashboard.getShareToken();
        if (token == null || token.isEmpty()) {
            token = UUID.randomUUID().toString().replace("-", "");
        }

        DashboardDO update = new DashboardDO();
        update.setId(id);
        update.setShareToken(token);
        update.setIsPublic(YesOrNoEnum.YES.getLetter());
        update.setGmtModified(new java.util.Date());
        getMapper().updateById(update);

        return DataResult.of(token);
    }

    @Override
    public ActionResult disableShare(Long id) {
        Dashboard dashboard = queryExistent(id).getData();
        PermissionUtils.checkOperationPermission(dashboard.getUserId());

        DashboardDO update = new DashboardDO();
        update.setId(id);
        update.setIsPublic(YesOrNoEnum.NO.getLetter());
        update.setGmtModified(new java.util.Date());
        getMapper().updateById(update);
    ...
    @Override
    public DataResult<Dashboard> findByShareToken(String shareToken) {
        LambdaQueryWrapper<DashboardDO> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper
            .eq(DashboardDO::getShareToken, shareToken)
            .eq(DashboardDO::getIsPublic, YesOrNoEnum.YES.getLetter())
            .eq(DashboardDO::getDeleted, YesOrNoEnum.NO.getLetter());
        List<DashboardDO> results = getMapper().selectList(queryWrapper);
        if (CollectionUtils.isEmpty(results)) {
            throw new DataNotFoundException();
        }
        Dashboard dashboard = dashboardConverter.do2model(results.get(0));
        LambdaQueryWrapper<DashboardChartRelationDO> relationWrapper = new LambdaQueryWrapper<>();
        relationWrapper.eq(DashboardChartRelationDO::getDashboardId, dashboard.getId());
        List<DashboardChartRelationDO> relationDO = getMapper1().selectList(relationWrapper);
        List<Long> chartIds = relationDO.stream().map(DashboardChartRelationDO::getChartId).toList();
        dashboard.setChartIds(chartIds);
        return DataResult.of(dashboard);
    }
