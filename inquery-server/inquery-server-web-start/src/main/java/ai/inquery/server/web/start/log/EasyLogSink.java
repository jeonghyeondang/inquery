package ai.inquery.server.web.start.log;

import ai.inquery.server.tools.common.util.LogUtils;
...
public class EasyLogSink implements Sink {
    ...
    public void printLog(final Correlation correlation, final HttpRequest request, final HttpResponse response)
        ...
        try {
            ...
            // Skip response body logging for large schema/catalog endpoints
            if (path.startsWith("/api/erd/") || path.startsWith("/api/schema-cache/") || path.startsWith("/api/catalog/") || path.startsWith("/api/operation/log")) {
                webLog.setResponse("[response body omitted, " + response.getBody().length + " bytes]");
            } else if (ContentTypeUtils.isContentTypeJSON(response.getContentType()) || ContentTypeUtils.isContentTypeHTML(
        ...
        if (StringUtils.isNotBlank(webLog.getQuery())) {
            String cleanedQuery = stripVerboseParams(webLog.getQuery());
            if (StringUtils.isNotBlank(cleanedQuery)) {
                pathAndQuery += "?" + cleanedQuery;
            }
        }
    ...
    /**
     * Strips verbose query parameters (excludedTables, satoken) from the query string to keep logs readable.
     */
    private String stripVerboseParams(String query) {
        if (query == null || query.isEmpty()) return query;
        StringBuilder result = new StringBuilder();
        for (String param : query.split("&")) {
            String key = param.contains("=") ? param.substring(0, param.indexOf('=')) : param;
            if ("excludedTables".equals(key) || "satoken".equals(key)) {
                continue;
            }
