package ai.inquery.server.web.api.controller.task.biz;

import ai.inquery.server.domain.api.enums.*;
import ai.inquery.server.domain.api.param.*;
import ai.inquery.server.domain.api.service.TableService;
import ai.inquery.server.domain.api.service.TaskService;
...
import ai.inquery.server.web.api.controller.rdb.RdbDmlExportController;
...
import ai.inquery.server.web.api.controller.rdb.factory.ExportServiceFactory;
import ai.inquery.server.web.api.controller.rdb.request.DataExportRequest;
...
public class TaskBizService {
    ...
    public DataResult<Long> exportResultData(DataExportRequest request) {
        ...
        DbType dbType = JdbcUtils.parse2DruidDbType(InqueryContext.getConnectInfo().getDbType());
        ...
        ConnectInfo connectInfo = InqueryContext.getConnectInfo().copy();
    ...
    private void doExportCsv(String sql, File file) {
        ...
        try {
            ...
            SQLExecutor.getInstance().execute(InqueryContext.getConnection(), sql, headerList -> {
    ...
    private void doExportInsert(String sql, File file, DbType dbType,
        ...
        try (PrintWriter printWriter = new PrintWriter(file, StandardCharsets.UTF_8.name())) {
            ...
            String databaseName = InqueryContext.getConnectInfo().getDatabaseName();
            String schemaName = InqueryContext.getConnectInfo().getSchemaName();
            List<String> headerColumns = Lists.newArrayList();
            SQLExecutor.getInstance().execute(InqueryContext.getConnection(), sql,
                    headerList -> {
