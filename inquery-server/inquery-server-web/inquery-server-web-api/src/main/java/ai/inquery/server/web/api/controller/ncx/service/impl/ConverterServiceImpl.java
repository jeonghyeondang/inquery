package ai.inquery.server.web.api.controller.ncx.service.impl;
...
import ai.inquery.server.web.api.controller.ncx.cipher.CommonCipher;
import ai.inquery.server.web.api.controller.ncx.dbeaver.DefaultValueEncryptor;
import ai.inquery.server.web.api.controller.ncx.enums.DataBaseType;
import ai.inquery.server.web.api.controller.ncx.enums.ExportConstants;
import ai.inquery.server.web.api.controller.ncx.enums.VersionEnum;
import ai.inquery.server.web.api.controller.ncx.factory.CipherFactory;
import ai.inquery.server.web.api.controller.ncx.service.ConverterService;
import ai.inquery.server.web.api.controller.ncx.vo.UploadVO;
import ai.inquery.server.web.api.util.XMLUtils;
...
public class ConverterServiceImpl implements ConverterService {
    ...
    public UploadVO uploadFile(File file) {
        ...
        try {
            ...
            // Get the link imported from navicat and write it into the h2 database of inquery
            ...
            if (projectsElement != null) {
                ...
                for (Element projectElement : projectList) {
                    ...
                    for (String key : keys) {
                        ...
                        //The database type is not matched. For example: dbeaver supports custom drivers, etc., but inquery does not support it yet.
    ...
    public UploadVO datagripUploadFile(String text) {
        ...
        if (!text.startsWith(DATASOURCE_SETTINGS)) {
            throw new RuntimeException("Connection information header is incorrect!");
        }
    ...
    public void insertDBConfig(List<Map<String, Map<String, String>>> list) {
        for (Map<String, Map<String, String>> map : list) {
            for (Map.Entry<String, Map<String, String>> valueMap : map.entrySet()) {
                ...
                DataSourceDO dataSourceDO;
                if (null == dataBaseType) {
                    //The database type is not matched. For example: navicat supports MongoDB, etc., but inquery does not support it yet.
                    continue;
                } else {
                    dataSourceDO = new DataSourceDO();
                ...
                //Insert user id
                dataSourceDO.setUserId(ContextUtils.getUserId());
                //Password is the decrypted ciphertext, and then uses inquery for encryption.
                DesUtil desUtil = new DesUtil(DesUtil.DES_KEY);
                ...
                if ("false".equals(resultMap.get("SSH"))) {
                    ...
                    // Currently inquery only supports password and Private key
