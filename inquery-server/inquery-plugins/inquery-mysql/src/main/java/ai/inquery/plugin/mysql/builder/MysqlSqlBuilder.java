package ai.inquery.plugin.mysql.builder;

import ai.inquery.plugin.mysql.type.MysqlColumnTypeEnum;
import ai.inquery.plugin.mysql.type.MysqlIndexTypeEnum;
import ai.inquery.spi.enums.EditStatus;
import ai.inquery.spi.jdbc.DefaultSqlBuilder;
...
public class MysqlSqlBuilder extends DefaultSqlBuilder {
    ...
    public static List<TableColumn> movedElements(List<TableColumn> original, List<TableColumn> modified) {
        ...
        // Track LCS, find elements that have been moved
        ...
        while (i > 0 && j > 0) {
            if (original.get(i - 1).equals(modified.get(j - 1))) {
                ...
                // Find the position of original.get(i-1) in the modified list
//                System.out.println("Moved elements:"+ original.get(i-1).getName() + " after " + modified.indexOf(original.get(i-1)) );
        ...
        // Add elements from the original list that are not included in the LCS
        while (i > 0) {
            moved.add(original.get(i - 1));
            i--;
        }
    ...
    private static String[] moveElement(String[] originalArray, int from, int to, String[] targetArray, AtomicInteger continuousDataCount) {
        ...
        // Whether there is continuous move data
        boolean isContinuousData = false;
        // Continuous data count
        if (from < to) {
