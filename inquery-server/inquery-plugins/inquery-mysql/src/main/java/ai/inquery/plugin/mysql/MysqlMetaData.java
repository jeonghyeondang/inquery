...
import ai.inquery.plugin.mysql.builder.MysqlSqlBuilder;
import ai.inquery.plugin.mysql.type.*;
import ai.inquery.plugin.mysql.value.MysqlValueProcessor;
...
import static ai.inquery.spi.util.JdbcJarUtils.getFullPath;
...
public class MysqlMetaData extends DefaultMetaService implements MetaData {
    ...
    public List<Database> databases(Connection connection) {
        ...
        return ai.inquery.spi.util.SortUtils.sortDatabase(databases, systemDatabases, connection);
    }
    ...
    private static String SELECT_ALL_COLUMNS = "SELECT * FROM information_schema.COLUMNS  WHERE TABLE_SCHEMA =  '%s'  order by TABLE_NAME, ORDINAL_POSITION";

    @Override
    public List<TableColumn> columns(Connection connection, String databaseName, String schemaName, String tableName) {
        // If tableName is null or empty, fetch all columns in the schema (for ERD bulk loading)
        String sql;
        if (tableName == null || tableName.trim().isEmpty()) {
            sql = String.format(SELECT_ALL_COLUMNS, databaseName);
        } else {
            sql = String.format(SELECT_TABLE_COLUMNS, databaseName, tableName);
        }
        ...
        return SQLExecutor.getInstance().execute(connection, sql, resultSet -> {
            while (resultSet.next()) {
                ...
                // Get tableName from result if bulk loading, otherwise use parameter
                String actualTableName = (tableName == null || tableName.trim().isEmpty()) 
                    ? resultSet.getString("TABLE_NAME") 
                    : tableName;
                column.setTableName(actualTableName);
