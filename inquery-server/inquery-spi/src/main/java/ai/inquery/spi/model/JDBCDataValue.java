...
public class JDBCDataValue {
    ...
    public String getBinaryDataString() {
        ...
        try {
            ...
            // Check if stream supports mark operation, use BufferedInputStream if not
            ...
            long size = 0;
            byte[] buffer = new byte[8192]; // buffer area
            int bytesRead;
            while ((bytesRead = binaryStream.read(buffer)) != -1) {
                size += bytesRead;
            }
            binaryStream.reset(); // Reset stream to marked position
