...
public class SqlUtils {
    ...
    public static void buildCanEditResult(String sql, DbType dbType, ExecuteResult executeResult) {
        try {
            // Skip parsing for EXPLAIN queries - they are not editable and may use DB-specific syntax
            // (e.g., Snowflake's "EXPLAIN USING JSON" which JSQLParser doesn't support)
            ...
            // Skip JSQLParser for unsupported databases (e.g., BigQuery)
            // BigQuery uses hyphenated project IDs which JSQLParser cannot parse
            // When dbType is null, it means Druid doesn't support this database type
            if (dbType == null) {
                executeResult.setCanEdit(false);
                return;
            }
            ...
            // Use debug level since parsing failures are expected for some databases
            // (e.g., BigQuery with hyphenated identifiers, DB-specific syntax)
            log.debug("buildCanEditResult parsing failed, setting canEdit=false: {}", e.getMessage());
            executeResult.setCanEdit(false);
        }
    }
...
//            sql = removeDelimiter(sql);
    ...
    private static String removeDelimiter(String str) {
        try {
            if (str.toUpperCase().contains("DELIMITER")) {
                ...
                while (matcher.find()) {
                    // Get and print the first string after "DELIMITER"
    ...
    private static List<String> splitWithCreateEvent(String str, DbType dbType) {
        ...
        int lastEnd = 0; // Track the end position of the previous match
        while (matcher.find()) {
            ...
            lastEnd = matcher.end(); // Update the end position of the previous match
        }
    ...
    private static final String DEFAULT_VALUE = "INQUERY_UPDATE_TABLE_DATA_USER_FILLED_DEFAULT";
    ...
    public static String quoteObjectName(String name, String quoteSymbol) {
        if (StringUtils.isNotBlank(name)) {
            ...
            if (!startsWithQuote && !endsWithQuote) {
                // If there are no quote symbols at the beginning and end
                return quoteSymbol + name + quoteSymbol;
            } else if (startsWithQuote && !endsWithQuote) {
                // If there is a quote symbol only at the beginning
                return quoteSymbol + quoteSymbol + name + quoteSymbol;
            } else if (!startsWithQuote) {
                // If there is a quote symbol only at the end
                return quoteSymbol + name + quoteSymbol + quoteSymbol;
            }
            // If there are quote symbols at both the beginning and end, return the original string
            return name;
        }
        // If name is null or only contains whitespace, return the original string
        return name;
    }
