...
public class SqlSplitter {
    ...
    /**
     * Token patterns used to detect PL block start
     */
    private final int[][] ALL_PL_START_PATTERNS;

    /**
     * Token types for removing SQL statement prefix comments
     */
    private final int[] BLANK_OR_COMMENT_TYPES;

    /**
     * Token types to ignore when detecting PL block start
     */
    private final int[] PL_START_PATTERN_IGNORE_TYPES;

    /**
     * Token types for IDENT
     */
    private final int[] PL_IDENT_TYPES;

    /**
     * Cached token type list for detecting PL block entry.
     * Uses at most MAX_PL_PATTEN_TYPE_SIZE tokens for performance; beyond that is not a PL block.
     */
    private final List<Integer> cacheTokenTypes = new ArrayList<>();

    /**
     * Current statement buffer; cleared when a statement ends for the next one
     */
    private final StringBuilder currentStmtBuilder = new StringBuilder();

    /**
     * Split statement results
     */
    ...
    /**
     * Current statement state; starts as SQL_STMT, switches to PL_STMT when entering a PL block
     */
    private State state = State.SQL_STMT;

    /**
     * Current delimiter; initially the session delimiter, written back when split completes
     */
    private String delimiter;

    /**
     * Tokens matching the delimiter; recomputed when delimiter changes to avoid repeated parsing
     */
    private Token[] delimiterTokens;

    /**
     * Whether to remove comment prefix; workaround for OB 3.1.x PL statements with comment prefix errors
     */
    @Getter
    @Setter
    ...
    /**
     * Tracks sub-block nesting within PL block
     */
    private Stack<SubPLLevel> subPLStack = new Stack();

    /**
     * Cached token types in PL block for detecting sub-PL block entry.
     * Uses at most MAX_PL_PATTEN_TYPE_SIZE tokens for performance; beyond that is not a PL block.
     */
    private List<Integer> plCacheTokenTypes = new ArrayList<>();

    /**
     * Maps PL block start pattern index in ALL_PL_START_PATTERNS to its enum type
     */
    ...
    public List<SplitSqlString> split(String sql) {
        ...
        /**
         * Token list after ANTLR Lexer tokenization
         */
    ...
    class SubPLLevel {
        ...
        /**
         * Records whether EXTERNAL or LANGUAGE appeared in PL DDL (e.g. create package/function).
         * If so, the DDL must end with delimiter only.
         */
        private boolean matchExternalOrLanguage = false;
        /**
         * Records whether IS or AS appeared in PL DDL (e.g. create package/function)
         */
        private boolean matchIsOrAs = false;
        /**
         * Records whether MEMBER or STATIC appeared in the type object
         */
    ...
    class InnerUtils {
        ...
        /**
         * Oracle lexer requires IDENT to start with a letter; in "delimiter $$", $$ is treated as invalid.
         * Converts unrecognized tokens to IDENT and SPACES for consistent handling upstream.
         */
