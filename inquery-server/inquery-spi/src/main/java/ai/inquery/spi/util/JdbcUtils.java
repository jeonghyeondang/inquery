...
public class JdbcUtils {
    ...
    public static DataSourceConnect testConnect(String url, String host, String port,
        ...
        try {
            ...
            // Special handling for BigQuery - build URL with authentication parameters
            if ("BIGQUERY".equalsIgnoreCase(dbType) && properties != null) {
                url = buildBigQueryUrl(url, properties);
            }
        ...
        dataSourceConnect.setDescription("Success");
        return dataSourceConnect;
    }
     ...
     */
    private static String buildBigQueryUrl(String baseUrl, Map<String, Object> properties) {
        String projectId = (String) properties.get("projectId");
        String serviceAccountJson = (String) properties.get("serviceAccountJson");
        String defaultDataset = (String) properties.get("defaultDataset");

        if (StringUtils.isBlank(projectId) || StringUtils.isBlank(serviceAccountJson)) {
            log.warn("BigQuery: projectId or serviceAccountJson is missing, returning original URL");
            return baseUrl;
        }
        ...
        if (StringUtils.isBlank(serviceAcctEmail)) {
            log.warn("BigQuery: client_email not found in serviceAccountJson");
            return baseUrl;
        }

        try {
            // Create temporary file for service account JSON
            java.nio.file.Path tempFile = java.nio.file.Files.createTempFile("bigquery_test_sa_", ".json");
            tempFile.toFile().deleteOnExit();
            java.nio.file.Files.writeString(tempFile, serviceAccountJson);
            String keyFilePath = tempFile.toAbsolutePath().toString();
            ...
            log.info("BigQuery test connection URL built successfully");
            return url.toString();
        } catch (Exception e) {
            log.error("Failed to build BigQuery URL: {}", e.getMessage());
            return baseUrl;
        }
    }
     ...
     */
    private static String extractClientEmail(String serviceAccountJson) {
        java.util.regex.Pattern pattern = java.util.regex.Pattern.compile("\"client_email\"\\s*:\\s*\"([^\"]+)\"");
        java.util.regex.Matcher matcher = pattern.matcher(serviceAccountJson);
        if (matcher.find()) {
